...
app.post('/api/admin/login', async (req, res) => {
  ...
  try {
    ...
    // 1. First, attempt to query by the "username" field in Firestore
    const adminsCol = collection(db, 'admins');
    const q = query(adminsCol, where('username', '==', trimmedUsername));
    const querySnapshot = await getDocs(q);

    if (!querySnapshot.empty) {
      adminDocData = querySnapshot.docs[0].data();
    } else {
      // 2. Fallback: Check if there's a document with the ID matching trimmedUsername
      const adminDocRef = doc(db, 'admins', trimmedUsername);
    ...
    // 3. Password Verification: support both hashed (bcrypt) and plain-text (e.g. "HOS")
    let isMatch = false;

    if (adminDocData.passwordHash) {
      isMatch = await bcrypt.compare(password, adminDocData.passwordHash);
    }

    if (!isMatch && adminDocData.password) {
      isMatch = (adminDocData.password === password);
    }
    ...
    const tokenUsername = adminDocData.username || trimmedUsername;
    const token = jwt.sign(
      { username: tokenUsername, role: adminDocData.role || 'admin' },
      JWT_SECRET,
      { expiresIn: '24h' }
    );
